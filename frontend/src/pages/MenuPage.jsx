...
import { ClipboardList, Search, Plus, Coffee, Tag, Banknote, Edit } from 'lucide-react';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
              {products.map((item) => (
                <Card key={item.id} className="clay-card overflow-hidden flex flex-col group relative">
                  <Button
                    variant="secondary"
                    className="absolute top-4 left-4 z-10 w-10 h-10 rounded-full bg-white/80 backdrop-blur-md text-primary hover:bg-white shadow-lg opacity-0 group-hover:opacity-100 transition-opacity p-0 flex items-center justify-center"
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex justify-between items-center text-xs font-bold text-muted-foreground bg-muted/10 p-3 rounded-xl">
                       <span>HPP:</span>
                       <span>Rp {item.cost_price.toLocaleString('id-ID')}</span>
                    </div>
                  </div>
                </Card>
              ))}
